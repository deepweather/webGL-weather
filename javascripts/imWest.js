$(document).ready(function() {	
	var controlContainer = L.Control.extend({
		...
		onAdd: function(map) {
			...
			this.txtFld = L.DomUtil.create('p', 'leaflet-custom-text '+this.options.txtClass, container);
	...
	holdUp = true;
	$.ajax({
		url: "https://qmetdev-bucket.fra1.digitaloceanspaces.com/ut-data/ut_data-"+forecast+"-"+showVar+".json",
	...
	function updateColor(direction) {
		...
		var tstart = new Date();
		if (showVar === 'prec') {
			var getUrl = "https://qmetdev-bucket.fra1.digitaloceanspaces.com/ut-data/ut_data-"+pfcst+"-"+showVar+".json"
		} else {
			var getUrl = "https://qmetdev-bucket.fra1.digitaloceanspaces.com/ut-data/ut_data-"+forecast+"-"+showVar+".json"
		}
		$.ajax({
			url: getUrl,
			success: function(dat) {
